...
public struct Alert {
    ...
    public static func showWithCancel(title: String? = nil, message: String? = nil, button: Button, cancelButtonTitle: String = "キャンセル", on viewController: UIViewController) {
        let alertController = UIAlertController(title: title, message: message, preferredStyle: .alert)
        alertController.addAction(UIAlertAction(title: button.title, style: button.style, handler: { _ in button.action() }))
